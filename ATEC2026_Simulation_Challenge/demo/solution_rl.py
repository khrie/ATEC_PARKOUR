...
class AlgSolution:
    ...
    def _extract_policy_obs(self, obs, action_dim) -> torch.Tensor:
        ...
        extero = obs.get("extero", torch.zeros((proprio.shape[0], 5760), device=self.device)).to(self.device)
        ...
        policy_obs = torch.cat([policy_obs_proprio, extero], dim=-1)
