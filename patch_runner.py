...
content = content.replace("class OnPolicyRunnerWithExtractor(OnPolicyRunner):", helper + "class OnPolicyRunnerWithExtractor(OnPolicyRunner):")

content = content.replace('obs = obs_dict["policy"] if "policy" in obs_dict.keys() else obs_dict', 'obs = _extract_obs(obs_dict)')

# Also fix the one occurrence where it uses batch.observations
